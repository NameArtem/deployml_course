...
def test_split(df_agg, random_state):
    try:
        ...
        assert (len(split(df_agg, random_state)[0]) + len(split(df_agg, random_state)[3])) == len(df_agg)
        ...
        # идекса из train нет в тест
        for i in split(df_agg, random_state)[3].index.values:
            assert i not in split(df_agg, random_state)[0].index.values
